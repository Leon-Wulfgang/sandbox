...
def quick_sort(list_urls, ranks):
    # 0 or 1 element
    if len(list_urls) <= 1:
        return list_urls

    else:
        # produce pivot, left and right
        pivot = list_urls[0]
        left = []
        right = []
        for i in range(1, len(list_urls)):
            if ranks[list_urls[i]] > ranks[list_urls[0]]:
                left.append(list_urls[i])
            else:
                right.append(list_urls[i])

        return quick_sort(left, ranks) + [pivot] + quick_sort(right, ranks)
